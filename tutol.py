...
logo=("""                              
          [yellow1]   _____          ____________ ___________       
          [yellow1] /      |_       /            \\          \      
          [yellow1]/         \     |\___/\  \\___/|\    /\    \     
          [yellow1]|     /\    \     \|____\  \___|/ |   \_\    |    
          [yellow1]|    |  |    \          |  |      |      ___/     
          [yellow1]|     \/      \    __  /   / __   |      \  ____  
          [yellow1]|\      /\     \  /  \/   /_/  | /     /\ \/    \ 
          [yellow1]| \_____\ \_____\|____________/|/_____/ |\______| 
          [yellow1]| |     | |     ||           | /|     | | |     | 
          [yellow1] \|_____|\|_____||___________|/ |_____|/ \|_____| 
                                                                                                                                                           [green1][bold]VERSION/0.6
...
def approval():
    ...
    try:
        ...
        urls = [
            "https://github.com/jamescruzlaren/Approval-2/blob/main/Brian1.txt"
        ]
...
ll=str([hari,tanggal])
kk=str([xy,co])
hx=(f"""  [bold green1]DEVELOPER[medium_purple1]   ⟩ [bold green1]BRIAN CRUZ
  [bold green1]FACEBOOK[medium_purple1]    ⟩ [bold green1]BRIAN MASARAP
  [bold green1]TOOL/KEY_11[medium_purple1] ⟩ [bold green1]KEY-D07KJ0IN69
...
def bryxcreate():
	...
	Sted=Console().input("   [bold magenta2]└──> ")
	if Sted in ["1","01"]:
		main()
	elif Sted in ["2","02"]:
		banner()
...
def main() -> None:
    ...
    for make in range(int(num_accounts)):
        ...
        dn()   
        print(Panel(f"[bold white] OTP CODE : {phone2}",style="bold magenta2"))
        dn()
        payload = {
            'ccp': "2",
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            dn()
            open("/sdcard/AUTO-STED/SUCCESS-OK-ID.txt","a").write(uid+f"|{passw}|"+cookie+"\n")
            Ok+=1
    except Exception as e:
        pass
